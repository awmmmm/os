    ...
    if((ptep = get_pte(mm->pgdir,addr,1))==NULL)         //(1) try to find a pte, if pte's PT(Page Table) isn't existed, then create a PT.
           ...
           //    	struct Page *page =
               ...
               */
                   if(swap_init_ok) {
                       struct Page *page=NULL;
                       int s_i = swap_in(mm,addr, &page);                        //(1）According to the mm AND addr, try to load the content of right disk page
                                               //    into the memory which page managed.
       //                cprintf(" swap_out_page is: 0x%x\n",page->pra_vaddr);
                       page_insert(mm->pgdir,page,addr,perm);
           //            cprintf(" addr is: 0x%x\n",addr);//(2) According to the mm, addr AND page, setup the map of phy addr <---> logical addr

                       swap_map_swappable(mm, addr, page, s_i);                      //(3) make the page swappable.
                       page->pra_vaddr=addr;
                   }
                   else {
                       cprintf("no swap_init_ok but ptep is %x, failed\n",*ptep);
