...
swap_out(struct mm_struct *mm, int n, int in_tick)
{
     ...
     for (i = 0; i != n; ++ i)
     {
          ...
          int r = sm->swap_out_victim(mm, &page, in_tick);
           cprintf("i %d, SWAP: call swap_out_victim , page_va: 0x%x \n",i,page->pra_vaddr);
          if (r != 0) {
          ...
          if (swapfs_write( (page->pra_vaddr/PGSIZE+1)<<8, page) != 0) {
                    ...
                    *ptep = (page->pra_vaddr/PGSIZE+1)<<8;
                    free_page(page);
                    cprintf("i %d, swap_out: page_pa: 0x%x\n",i,page2pa(page));
                    list_entry_t *le = &free_list;
                    while ((le = list_next(le)) != &free_list){
                    	cprintf("freelist, swap_out: 0x%x\n",page2pa(le2page(le,page_link)));
                    	cprintf("freeblock_size, swap_out: 0x%x\n",le2page(le,page_link)->property);
                    }
                    cprintf("##########\n");
          }
...
swap_in(struct mm_struct *mm, uintptr_t addr, struct Page **ptr_result)
{
     ...
     cprintf("swap_in: alloc_page pa: 0x%x\n",page2pa(result));
     cprintf("swap_in: alloc_page va: 0x%x\n",result->pra_vaddr);
     assert(result!=NULL);
