...
//       size_t size, alen = 0;
//       uint32_t ino;
//    uint32_t disk_block_index = 0;
    blkoff = offset % SFS_BLKSIZE;
    size = (nblks != 0) ? (SFS_BLKSIZE - blkoff) : (endpos - offset);
  if(blkoff != 0){
	  if (ret =sfs_bmap_load_nolock(sfs,sin,blkno,&ino)!= 0)//find disk_inode
		  goto out;

	  sfs_buf_op(sfs,buf,size,ino,blkoff);//read first block
	  alen += size;
	  offset += size;
 ...
 if(nblks > 0)
 {
	 if (ret =sfs_bmap_load_nolock(sfs,sin,blkno,&ino)!= 0)
		 goto out;
	 sfs_block_op(sfs,buf,ino,nblks);//read nblks block
	 alen += nblks*size;
	 offset += nblks*size;
	 blkno = offset / SFS_BLKSIZE;
	 	  nblks = endpos / SFS_BLKSIZE - blkno;
